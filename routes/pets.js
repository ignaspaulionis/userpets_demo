...
const VALID_PET_TYPES = ['dog', 'cat', 'bird', 'fish', 'hamster'];

const isValidId = (value) => Number.isInteger(Number(value)) && Number(value) > 0;
const isNonEmptyString = (value) => typeof value === 'string' && value.trim().length > 0;

router.get('/types', (req, res) => {
  res.status(200).json({ types: VALID_PET_TYPES });
});
...
router.post('/', async (req, res) => {
  try {
    ...
    // Validate type
    if (!type || typeof type !== 'string' || !VALID_PET_TYPES.includes(type.toLowerCase())) {
      return res.status(400).json({ error: `Type must be one of: ${VALID_PET_TYPES.join(', ')}` });
    }
